...
import { getPendingResearchTasks, getSubAdminCategories } from '../../api/subadmin.api';
...
export default function SubAdminResearchReview(): JSX.Element {
  ...
  const isLinkedInRoute = location.pathname.includes('linkedin-research');
  const platformFilter: PlatformFilter = isLinkedInRoute ? 'linkedin' : 'website';
  
  const [tasks, setTasks] = useState<ResearchAuditTask[]>([]);
  ...
  // Detail modal
  const [selectedTask, setSelectedTask] = useState<ResearchAuditTask | null>(null);
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        const result = await getPendingResearchTasks(limit, offset);
        ...
        filtered = filtered.filter(task => task.source === platformFilter);
        ...
        console.error('Error loading research audit tasks:', error);
        setLoadState('error');
      }
    };
  ...
  const tasksByCategory = tasks.reduce(
    ...
    {} as Record<string, ResearchAuditTask[]>,
  );
  ...
  return (
    <div className="website-research-container">
      <div className="website-research-header">
        <h1>🔍 Research Audit Queue</h1>
        <p className="header-subtitle">
          Review and audit submitted research tasks before they proceed to inquiry phase
        </p>
      </div>
      ...
      {loadState === 'loading' && (
        <div className="loading-container">
          ...
          <p>Loading research audit tasks...</p>
        </div>
      )}
      ...
      {loadState === 'error' && (
        <div className="error-container">
          <p>❌ Failed to load research audit tasks</p>
      ...
      {loadState === 'success' && tasks.length === 0 && (
        <div className="empty-state">
          <p>📭 No research tasks awaiting audit</p>
                  ...
                  <div key={task.id} className="task-card">
                    ...
                    <div className="task-card-body">
                      <div className="task-field">
                        ...
                        <span className="task-value">{getPlatformLabel(task.source)}</span>
                      </div>
            ...
            <div className="modal-body">
              ...
              <div className="detail-grid">
                <div className="detail-section">
                  ...
                  <p>{getPlatformLabel(selectedTask.source)}</p>
                </div>
