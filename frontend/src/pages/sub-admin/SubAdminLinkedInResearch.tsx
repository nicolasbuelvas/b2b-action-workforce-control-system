...
import { getLinkedInResearchTasks, getSubAdminCategories } from '../../api/subadmin.api';
...
interface LinkedInResearchTask extends ResearchItem {
  id: string;
  profileUrl: string;
  profileName?: string;
...
export default function SubAdminLinkedInResearch(): JSX.Element {
  ...
  // Detail modal
  const [selectedTask, setSelectedTask] = useState<LinkedInResearchTask | null>(null);
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        const result = await getLinkedInResearchTasks(
          categoryId,
          statusFilter,
          limit,
          offset,
        ...
        if (searchQuery.trim()) {
          ...
          filtered = tasksArray.filter(
            (task) =>
              (task.profileName && task.profileName.toLowerCase().includes(query)) ||
              (task.profileCompany && task.profileCompany.toLowerCase().includes(query)) ||
              (task.profileUrl && task.profileUrl.toLowerCase().includes(query)) ||
        ...
        console.error('Error loading LinkedIn research tasks:', error);
        setLoadState('error');
      }
    };
  ...
  const tasksByCategory = tasks.reduce(
    ...
    {} as Record<string, LinkedInResearchTask[]>,
  );
  ...
  return (
    <div className="website-research-container">
      <div className="website-research-header">
        <h1>💼 LinkedIn Research Tasks</h1>
        <p className="header-subtitle">
          View and manage all LinkedIn profile researcher submissions for your assigned categories
        </p>
      </div>
      ...
      <div className="website-research-filters">
        <div className="filters-grid">
          ...
          <div className="filter-group">
            <label htmlFor="search-filter">Search by Profile or Company</label>
            <input
              id="search-filter"
              type="text"
      ...
      {loadState === 'loading' && (
        <div className="loading-container">
          ...
          <p>Loading LinkedIn research tasks...</p>
        </div>
      )}
      ...
      {loadState === 'error' && (
        <div className="error-container">
          <p>❌ Failed to load LinkedIn research tasks</p>
      ...
      {loadState === 'success' && tasks.length === 0 && (
        <div className="empty-state">
          <p>📭 No LinkedIn research tasks found</p>
                  ...
                  <div key={task.id} className="task-card">
                    <div className="task-card-header">
                      <h3 className="task-company-name">
                        {task.profileName || task.profileCompany || 'LinkedIn Profile'}
                      </h3>
                      <span
                    ...
                    <div className="task-card-body">
                      <div className="task-field">
                        <span className="task-label">Profile URL:</span>
                      ...
                      {task.profileCompany && (
                        <div className="task-field">
                          <span className="task-label">Company:</span>
                          <span className="task-value">🏢 {task.profileCompany}</span>
                        </div>
                      )}
        ...
        <div className="modal-overlay" onClick={() => setSelectedTask(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{selectedTask.profileName || selectedTask.profileCompany || 'LinkedIn Profile'}</h2>
              <button 
            ...
            <div className="modal-body">
              ...
              <div className="detail-grid">
                {selectedTask.profileName && (
                  <div className="detail-section">
                    <label>Profile Name</label>
                    <p>{selectedTask.profileName}</p>
                  </div>
                )}

                {selectedTask.profileCompany && (
                  <div className="detail-section">
                    <label>Company</label>
                    <p>🏢 {selectedTask.profileCompany}</p>
                  </div>
                )}
